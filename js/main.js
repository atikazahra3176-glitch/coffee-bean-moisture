...
function generateAnalysis() {
  ...
  analysisText.innerHTML = `
    ...
    <strong>Jawaban: <span style="color:#0b6b3a; font-weight:800">TIDAK</span></strong><br><br>
    <strong>Penjelasan Berdasarkan Logika Fisik & Dataset:</strong><br>
    Dari pengamatan umum dan dataset 34 provinsi di Indonesia, secara logis dapat disimpulkan:<br><br>

    <strong>1. Prinsip Dasar (logika fisik):</strong><br>
    • <strong>Kelembapan lingkungan tinggi</strong> biasanya menghasilkan <strong>kadar air biji lebih tinggi</strong> pada saat penyimpanan; biji dengan kadar air lebih tinggi cenderung membutuhkan waktu lebih lama untuk kehilangan kelembapan saat pemanggangan, sehingga <strong>risiko menjadi over-roasted menurun</strong> pada pengaturan roasting standar.<br>
    • Sebaliknya, <strong>kelembapan lingkungan rendah</strong> → <strong>kadar air biji lebih rendah</strong> → biji lebih cepat kehilangan kelembapan saat roasting, sehingga <strong>risiko over-roasted meningkat</strong> jika parameter roasting tidak disesuaikan.<br><br>

    <strong>2. Observasi Dataset & Anomali:</strong><br>
    • Rata-rata Over Roast di seluruh provinsi: <strong>${avgOverroast}%</strong> (jangkauan: ${minOverroast}% - ${maxOverroast}%)<br>
    • Provinsi dengan humidity >80% (rata-rata over-roast ${(coffeeData.filter(d=>d.humidity>80).reduce((s,d)=>s+d.overroast_pct,0)/Math.max(1,coffeeData.filter(d=>d.humidity>80).length)).toFixed(1)}%) dan dengan humidity <60% (rata-rata over-roast ${(coffeeData.filter(d=>d.humidity<60).reduce((s,d)=>s+d.overroast_pct,0)/Math.max(1,coffeeData.filter(d=>d.humidity<60).length)).toFixed(1)}%) menunjukkan variasi; beberapa provinsi dengan kelembapan tinggi tetap mencatat over-roast relatif tinggi.
    <br>• Anomali ini kemungkinan disebabkan oleh variabel operasional lain seperti metode pengolahan pasca-panen, pengaturan roaster (suhu, aliran udara), durasi roasting, dan praktik penyimpanan lokal — yang semuanya dapat mengubah hubungan sederhana kelembapan→kadar air→hasil roasting.<br><br>

    <strong>3. Data Bukti (contoh):</strong><br>
    ...
    Secara fisik dan logis: <strong>kelembapan tinggi → kadar air lebih tinggi → cenderung NORMAL</strong>, dan <strong>kelembapan rendah → kadar air lebih rendah → risiko Over-Roasted lebih tinggi</strong> jika roasting tidak disesuaikan. Namun, <strong>kadar air bukanlah satu-satunya penentu</strong>; over-roasted ditentukan oleh kombinasi faktor: kadar air awal, parameter roasting, durasi, serta praktik pengolahan dan penyimpanan lokal.
  `;
}
